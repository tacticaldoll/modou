...
For policy drift, propose an **amendment**: a PR that changes the relevant
constitution with the reasoning, for the **steward** to accept or reject. Modou's own
self-governing constitution — the boundaries it enforces on itself — lives in
`crates/modou/tests/self_governance.rs`; `crates/modou/src/constitution.rs` is only the
bundled **demo** constitution (target `example-core`). Never edit either constitution,
the supply-chain policy (`deny.toml`), or the living specs in `openspec/specs/` to turn
CI green on your own.

This is enforced by convention, not by Modou: those paths —
`crates/modou/tests/self_governance.rs`, `crates/modou/src/constitution.rs`,
`deny.toml`, and `openspec/specs/` — are assigned to the steward in
`.github/CODEOWNERS`, and with required Code Owner review enabled on the default
...
Commit messages and PR descriptions carry **no AI/agent attribution** — no
`Co-Authored-By` trailer, and no tool-generated footer in any wording (e.g.
"Generated with …", "generated by …", "🤖 …"). The log records what changed and
why, not what tool typed it. Like the rule above, this is a convention, not a Modou
reaction: a commit's authorship line is not an observable architectural fact, so the
drift law keeps it out of the constitution.
...
`main` is **release-only**: it carries nothing but linear, non-merge
`release: X.Y.Z` snapshot commits, each tagged `vX.Y.Z`. The fine-grained
lifecycle commits above (`propose` / `apply` / `sync` / `archive`) never land on
`main` individually — they collapse through two squash stages on the way up (the
branch topology and the squash procedure are in `docs/development-flow.md`).

Both squashes are performed by a **GitHub pull request's "Squash and merge"**, not a
local merge: a change branch is squash-merged into `release/X.Y.Z`, and the release
branch is squash-merged into `main`. Strip GitHub's auto-appended `(#N)` from the
squash subject (the self-describing-commit rule above). A PR that touches a
steward-owned path (see `.github/CODEOWNERS`) is merged by the steward.

Like the self-describing commit rule above, this is a convention for humans and
agents, not a Modou reaction: a branching pattern is not an observable
architectural fact, so the drift law keeps it out of the constitution.
